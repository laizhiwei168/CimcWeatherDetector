package com.cimc.modbus.thread.pool;
...
public class SubscribePool {//LogThreadPool

	 private static SubscribePool threadPool;
	    ...
	    private long keepAliveTime =3000;     //线程没有执行任务时 被保留的最长时间 超过这个时间就会被销毁 直到线程数等于 corePoolSize
	  ...
	  //加锁
	    ...
	    public static SubscribePool init(){
	    	if (threadPool == null){
	            instanceLock.lock();
	            if(threadPool == null) {
	            	threadPool=new SubscribePool();
	            }
	            instanceLock.unlock();
	        }    	
	        return threadPool;
